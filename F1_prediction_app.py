...
def main():
    ...
    current_year = st.selectbox("Select the year you are predicting for:", options=[2023, 2024, 2025], index=4)
    ...
    race_mapping_prev = dict(zip(schedule_prev['EventName'], schedule.index))  # Map race name to round number
    # Get the round number based on the selected race
    ...
    if st.button("Predict Q3 Times"):
        ...
        st.write(f"Fetching data for {previous_year} round {race_num_prev}...")
        prev_year_data = fetch_f1_data(previous_year, race_num_prev)
        if prev_year_data is not None:
            prev_year_data['Year'] = previous_year
            prev_year_data['Round'] = race_num_prev
